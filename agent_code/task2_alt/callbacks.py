...
import agent_code.task2_alt.Setting as S
...
def setup(self):
    ...
    if self.train or not os.path.isdir("model"):
        ...
        self.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0002),loss=tf.keras.losses.Huber())
...
def act(self, game_state):
    ...
    if (self.epsilon >  rand or self.epsilon_min  > rand) and self.train:
        ...
        if not self.softmax:
            ...
            action = np.dot(np.arange(len(ACTIONS)),np.random.multinomial(1,action_probs))
...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    expand_f = np.ones(tuple(s+6 for s in field.shape))*-1     
    ...
    em_ac = expand_em[ownpos[0]+n-4:ownpos[0]+n+5,ownpos[1]+n-4:ownpos[1]+n+5]/4
