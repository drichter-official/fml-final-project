...
import agent_code.ac9x9_2.Setting as S
        ...
        if self.epsilon_imitate > rand and self.imitate:
            if not None in features:
                ...
                if Action is not None:
                    ...
                    return Action  
        if not self.softmax:
            self.logger.debug("Epsilon-greedy: Choosing action purely at random but only valids")
            if validactions(self, game_state) != [] : valacts = validactions(self, game_state)
            else: valacts = np.arange(len(ACTIONS))
            action = int(np.random.choice(valacts))
        else:
...
def validactions(self,game_state):
    ...
    if (x, y) in valid_tiles: valid_actions.append(4)
    # Disallow the BOMB action if agent dropped a bomb in the same spot recently
    if (bombs_left > 0) and (x, y) not in self.bomb_history: valid_actions.append(5)
...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    for bomb in game_state['bombs']:
        coinsandbombs.itemset(bomb[0],(1+bomb[1]))# dont normalize for emphasis on bombs
